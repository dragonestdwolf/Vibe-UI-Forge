/**
 * ScenarioModePageV5 — 情景模式
 *
 * 移动端设置页面下的二级页面
 *
 * 结构：
 *  - HeaderSlot: StatusBar + TitleBar
 *  - ContextSlot: 可滑动情景卡片（FeaturePromoCard + 导航点）
 *  - ListGroupSlot: 条件开启 list / 推荐开启与关闭 list
 *
 * 与 V4 差异：
 *  - 新增图标按钮：新增、问号详情、更多（TitleBar 右侧三个图标）
 *  - 调整布局细节
 */
...
function MoonIcon() {
  return (
    <svg viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      <defs>
        <linearGradient id="v5-moon-bg" x1="12%" y1="6%" x2="92%" y2="100%">
          <stop offset="0%" stopColor="#B69CFF" />
          <stop offset="100%" stopColor="#7B5CFF" />
        </linearGradient>
        <linearGradient id="v5-moon-shape" x1="20%" y1="10%" x2="80%" y2="100%">
          <stop offset="0%" stopColor="#FFFFFF" stopOpacity="0.95" />
          <stop offset="100%" stopColor="#E2D6FF" stopOpacity="0.9" />
        </linearGradient>
      </defs>
      <circle cx="32" cy="32" r="30" fill="url(#v5-moon-bg)" />
      <path
        d="M40.5 19c-.5 0-1 .03-1.5.08a14 14 0 0 0 6.4 23.43A15 15 0 1 1 40.5 19Z"
        fill="url(#v5-moon-shape)"
      />
...
function FocusIcon() {
  return (
    <svg viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      <defs>
        <linearGradient id="v5-focus-bg" x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%" stopColor="#FFD27A" />
          <stop offset="100%" stopColor="#FF8A4C" />
        </linearGradient>
      </defs>
      <circle cx="32" cy="32" r="30" fill="url(#v5-focus-bg)" />
...
function SleepIcon() {
  return (
    <svg viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      <defs>
        <linearGradient id="v5-sleep-bg" x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%" stopColor="#7DD3FC" />
          <stop offset="100%" stopColor="#3B82F6" />
        </linearGradient>
      </defs>
      <circle cx="32" cy="32" r="30" fill="url(#v5-sleep-bg)" />
...
function FootnoteText({ children }: { children: string }) {
  return <p className="smpv5-footnote">{children}</p>
}
...
export default function ScenarioModePageV5() {
      ...
      context={
        <>
          ...
          <div
            ...
            className="smpv5-rail smpv5-hide-scrollbar"
            role="list"
            aria-label="情景模式卡片列表"
          >
            {CARDS.map((card) => (
          ...
          <div className="smpv5-dots" role="tablist" aria-label="情景模式分页">
            {CARDS.map((card, idx) => (
              <span
                key={card.id}
                role="tab"
      ...
      listGroup={
        <>
          ...
          <div className="rounded-[20px] overflow-hidden" style={{ background: CARD_BG }}>
            ...
            <button type="button" className="smpv5-add-btn" onClick={() => {}}>
              添加条件
            </button>
          </div>

/**
 * ScenarioModePageV7 — 情景模式
 *
 * 移动端设置页面下的二级页面
 *
 * 结构：
 *  - HeaderSlot: StatusBar + TitleBar
 *  - ContextSlot: 可滑动情景卡片（FeaturePromoCard × 3 + 导航点）+ 描述文字
 *  - ListGroupSlot:
 *      Group-1 (条件开启): SubHeader + list-card + footnote
 *      Group-2 (智能推荐): SubHeader + list-card + footnote
 *
 * 与 V6 差异：
 *  - SubHeader 使用 my-subheader--text-none（移除底部 padding，符合 0-gap 规范）
 *  - 第一个 SubHeader 使用 my-subheader--above-dots（-12px 拉近导航点）
 *  - SubHeader + list-card + footnote 以 flex-col wrapper 分组，保证组内 0-gap
 *  - footnote 使用 footnote-text 类，由 layout CSS 统一管理 8px margin-top
 *  - 修复导航点 inactive 宽度缺失（width: 6px）
 */
...
function MoonIcon() {
  return (
    <svg viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      <defs>
        <linearGradient id="v7-moon-bg" x1="12%" y1="6%" x2="92%" y2="100%">
          <stop offset="0%" stopColor="#B69CFF" />
          <stop offset="100%" stopColor="#7B5CFF" />
        </linearGradient>
        <linearGradient id="v7-moon-shape" x1="20%" y1="10%" x2="80%" y2="100%">
          <stop offset="0%" stopColor="#FFFFFF" stopOpacity="0.95" />
          <stop offset="100%" stopColor="#E2D6FF" stopOpacity="0.9" />
        </linearGradient>
      </defs>
      <circle cx="32" cy="32" r="30" fill="url(#v7-moon-bg)" />
      <path
        d="M40.5 19c-.5 0-1 .03-1.5.08a14 14 0 0 0 6.4 23.43A15 15 0 1 1 40.5 19Z"
        fill="url(#v7-moon-shape)"
      />
...
function FocusIcon() {
  return (
    <svg viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      <defs>
        <linearGradient id="v7-focus-bg" x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%" stopColor="#FFD27A" />
          <stop offset="100%" stopColor="#FF8A4C" />
        </linearGradient>
      </defs>
      <circle cx="32" cy="32" r="30" fill="url(#v7-focus-bg)" />
...
function SleepIcon() {
  return (
    <svg viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      <defs>
        <linearGradient id="v7-sleep-bg" x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%" stopColor="#7DD3FC" />
          <stop offset="100%" stopColor="#3B82F6" />
        </linearGradient>
      </defs>
      <circle cx="32" cy="32" r="30" fill="url(#v7-sleep-bg)" />
...
export default function ScenarioModePageV7() {
  ...
  return (
    <SettingsContextListLayout
      ...
      context={
        <>
          {/* 描述文字，marginBlockEnd 16px（符合最小间距规范） */}
          <div style={{ paddingInline: 16, marginBlockEnd: 16 }}>
            <p style={{ margin: 0, fontSize: 13, lineHeight: "20px", color: FONT_TERTIARY }}>
          ...
          <div
            ...
            className="smpv7-rail smpv7-hide-scrollbar"
            role="list"
            aria-label="情景模式卡片列表"
          >
            {CARDS.map((card) => (
          ...
          {/* 导航点 */}
          <div className="smpv7-dots" role="tablist" aria-label="情景模式分页">
            {CARDS.map((card, idx) => (
              <span
                key={card.id}
                role="tab"
      ...
      listGroup={
        <>
          {/* ── Group 1: 条件开启 ── */}
          {/* marginTop: -12px 打在 wrapper 上，补偿父 gap=12px，让 SubHeader 贴近导航点 */}
          {/* SubHeader + list-card + footnote 分组，组内 gap=0，组间由父 flex gap=12 管理 */}
          <div className="flex flex-col" style={{ marginTop: -12 }}>
            ...
            <div className="rounded-[20px] overflow-hidden" style={{ background: CARD_BG }}>
              ...
              <button type="button" className="smpv7-add-btn" onClick={() => {}}>
                添加条件
              </button>
            </div>

            {/* margin-top: 8px 由 layout CSS .scl-layout__list-group .footnote-text 管理 */}
            <p
              className="footnote-text"
              style={{ fontSize: 12, lineHeight: "18px", color: FONT_TERTIARY }}
            >
              本功能需调用位置权限，读取位置信息。
            </p>
          </div>

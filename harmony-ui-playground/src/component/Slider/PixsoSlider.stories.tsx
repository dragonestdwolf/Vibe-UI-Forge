...
import { PixsoSlider, type PixsoSliderType } from "./PixsoSlider"
import { PixsoSeekbar, type PixsoSeekbarState } from "./PixsoSeekbar"
...
function centerInViewport(Story: () => ReactNode) {
  return (
    <div className="box-border flex min-h-screen w-full min-w-0 items-start justify-center bg-[#f3f4f6] p-8">
      <div className="w-[360px] rounded-[20px] bg-white p-3 shadow-sm">
        <Story />
      </div>
    </div>
  )
...
const meta = {
  title: "Components/Slider/PixsoSlider",
  component: PixsoSlider,
  tags: ["autodocs"],
...
} satisfies Meta<typeof PixsoSlider>
    ...
    return (
      <div className="flex flex-col gap-6">
        <Demo title="01. Basic — 基础" value={v} onChange={setV}>
          <PixsoSlider type="basic" value={v} onChange={setV} block />
        </Demo>

        <Demo title="02. Scale — 带刻度点" value={v} onChange={setV}>
          <PixsoSlider type="scale" value={v} onChange={setV} ticks={8} block />
        </Demo>
        ...
        <Demo title="05. Value — 底部刻度文案" value={v} onChange={setV}>
          <PixsoSlider type="value" value={v} onChange={setV} block />
        </Demo>
...
export const Seekbar: StoryObj<typeof PixsoSeekbar> = {
  name: "PixsoSeekbar / Playground",
  parameters: {
    docs: {
      description: {
        story:
  ...
  render: () => {
    ...
    return (
      <div className="flex flex-col gap-5">
        <PixsoSeekbar value={v} onChange={setV} block aria-label="进度" />
...
export const SeekbarStates: StoryObj<typeof PixsoSeekbar> = {
  name: "PixsoSeekbar / All States",
  render: () => (
    <div className="flex flex-col gap-5">
      {(
        ["default", "hover", "focus", "active", "disabled"] as PixsoSeekbarState[]
      ).map((s) => (
        <div key={s} className="flex flex-col gap-2">
          <span className="px-3 text-xs uppercase tracking-wider text-black/60">
            state = {s}

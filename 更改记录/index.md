...
+ TitleBarHarmony
  ...
  + | 存在问题 | 解决方案 |
    | ---- | ---- |
    |整体的宽度不对，设计稿是328px,设计组件是360px|设计组件的宽度改为328px|
    |中间部分主标题的行高设计稿是23，设计组件是20px|中间部分主标题的行高改为23|
    |中间部分副标题的行高设计稿是16px,设计组件是14px|中间部分副标题的行高改为16px|


### 优化点总结
+ 大部分设计组件的行高都有和设计稿对不上的情况，后续建议对这部分加以优化
+ 一部分组件的拓展性不足，比如状态栏组件右边的状态图标应该可以支持单个图标分别传入，但是设计组件只能传入一个整体的包含所有图标的图片文件
+ 一小部分组件的边距还没办法和设计稿保持完全一致

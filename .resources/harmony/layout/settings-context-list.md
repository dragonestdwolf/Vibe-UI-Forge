...
设置二级页通用布局：页面整体 `360px` 宽度居中呈现；顶部固定 StatusBar + TitleBar（居中于页面宽度内），二者之间 gap=0；中部为灵活业务卡片区（支持居中或满宽横滑）；底部为固定 list 卡片组合（卡片间距 12，可内嵌 slider）。适用于"先呈现/预览，后调参/控制"的设置二级页。支持 light / dark 双主题，背景色通过 `background` 参数控制。
